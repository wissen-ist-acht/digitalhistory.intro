...
`cats$total[1]`\
Im Katzenbeispiel sind es 134 Gesamttreffer, also `(134/10 ohne Rest)+1`, also `2` Durchgänge:\
`iterations <- cats$total[1]%/%10 + 1`\
...
`cat_list <- data.frame()`\
Wenn wir diese Variablen festgelegt haben, können wir einen Loop, eine Schlaufe bauen, die unter bestimmten Bedingungen verschiedene Aktionen ausführt:\
`for (i in 1:iterations){`\
