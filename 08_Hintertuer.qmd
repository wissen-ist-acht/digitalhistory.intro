...
```
``` bash
(base) serina00@dg-19-mac-02 Briefe % rm Brief1.txt
```
...
```
oder als bequeme Variante diesen [Direktlink](https://download-directory.github.io/?url=https://github.com/wissen-ist-acht/digitalhistory.intro/tree/main/docs/letters_Der_Sturm) nutzen.
...
Windows:
```bash
Invoke-WebRequest -Uri "https://sturm-edition.de/api/persons/P.0000003" -OutFile "briefe_marc.xml"
```
...
Windows:
```bash
Select-String -Path "briefe_marc.xml" -Pattern 'Q.*FMA.*\.xml\b' | ForEach-Object {
    $match = $_.Matches[0].Value
    "curl -o $match https://sturm-edition.de/api/files/$match"
} > dateinamen_briefe_marc.txt
```
...
`cat briefe_marc.xml` (Mac/Linux)/`-Path "briefe_marc.xml"` (Windows) gibt den Inhalt der Datei ins Terminal; 
`grep -o 'Q.*FMA.01.xml\b'` (Mac/Linux) bzw. `Select-String -Pattern 'Q.*FMA.*\.xml\b'` (Windows) findet in diesem Inhalt alle Zeichenketten zwischen "Q" und "FMA.01.xml", wobei nach "xml" durch das Hinzufügen von `\b` das Zeichenende angezeigt ist;
die 45 gefundenen Zeichenketten werden in je eine neue Zeile geschrieben, wobei mit `curl -o $_` der Befehl `curl -o` und mit `$_` (Mac/Linux) bzw. `$match` (Windows) als Platzhalter die Zeichenkette (also der Dateiname) geschrieben wird, gefolgt von  `https://sturm-edition.de/api/files/$_` (Mac/Linux) bzw. `https://sturm-edition.de/api/files/$match` und mit `$_` bzw. `$match` wieder die Zeichenkette (also wieder der Dateiname).
