...
`grep -o 'Q.*FMA.01.xml\b'` (Mac/Linux)/`Select-String -Pattern 'Q.*FMA.*\.xml\b'` (Windows) findet in diesem Inhalt alle Zeichenketten zwischen "Q" und "FMA.01.xml", wobei nach "xml" durch das Hinzufügen von `\b` das Zeichenende angezeigt ist;
die 45 gefundenen Zeichenketten werden in je eine neue Zeile geschrieben, wobei mit `curl -o $_` der Befehl `curl -o` und mit `$_` (Mac/Linux)/`$match` (Windows) als Platzhalter die Zeichenkette (also der Dateiname) geschrieben wird, gefolgt von  `https://sturm-edition.de/api/files/$_` (Mac/Linux)/`https://sturm-edition.de/api/files/$match` und mit `$_`/`$match` wieder die Zeichenkette (also wieder der Dateiname).
